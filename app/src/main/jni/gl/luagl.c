...
static int luagl_color(lua_State *L) {
    if (lua_gettop(L) == 4) {
        glColor4f(luaL_checknumber(L, 1), luaL_checknumber(L, 2), luaL_checknumber(L, 3),
...
static int luagl_light(lua_State *L) {
    if (lua_istable(L, 3)) {
        ...
        glLightf(luagl_get_gl_enum(L, 1), luagl_get_gl_enum(L, 2),
...
static int luagl_rotate(lua_State *L) {
    glRotatef(luaL_checknumber(L, 1), luaL_checknumber(L, 2), luaL_checknumber(L, 3),
              luaL_checknumber(L, 4));
    return 0;
}
...
static int luagl_scissor(lua_State *L) {
    glScissor(luaL_checkinteger(L, 1), luaL_checkinteger(L, 2), luaL_checkinteger(L, 3),
...
static int luagl_stencil_func(lua_State *L) {
    if (lua_type(L, 3) == LUA_TSTRING)
        glStencilFunc(luagl_get_gl_enum(L, 1), luaL_checkinteger(L, 2),
                      luagl_str2mask(luaL_checkstring(L, 3)));
    else
