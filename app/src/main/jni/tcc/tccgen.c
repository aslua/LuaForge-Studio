...
ST_FUNC int gv(int rc)
{
    ...
    if (vtop->type.t & VT_BITFIELD) {
        ...
        vtop->type.t &= ~(VT_BITFIELD | (0xFFFFFFFFu << VT_STRUCT_SHIFT));
...
ST_FUNC void vstore(void)
{
	...
	&& !(vtop->type.t & VT_BITFIELD)) {
        ...
        vtop->type.t = ft & (VT_TYPE & ~(VT_BITFIELD | (0xFFFFFFFFu << VT_STRUCT_SHIFT)));
        /* XXX: factorize */
        if (ft & VT_CONSTANT)
            tcc_warning("assignment of read-only location");
    } else {
    ...
    if (sbt == VT_STRUCT) {
        ...
        /* remove bit field info to avoid loops */
        vtop[-1].type.t = ft & ~(VT_BITFIELD | (0xFFFFFFFFu << VT_STRUCT_SHIFT));
