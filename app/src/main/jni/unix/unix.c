...
static u_error_t
u_sigtimedwait(int *_signo, const sigset_t *set, siginfo_t *_info, const struct timespec *timeout) {
...
static int
u_getgrgid_r(gid_t gid, struct group *grp, char *buf, size_t bufsiz, struct group **res) {
#if GETGRGID_R_AIX
                                                                                                                            if (bufsiz == 0)
		return ERANGE;
...
static int
u_getgrnam_r(const char *nam, struct group *grp, char *buf, size_t bufsiz, struct group **res) {
#if GETGRNAM_R_AIX
                                                                                                                            if (bufsiz == 0)
		return ERANGE;
...
static int
u_getpwuid_r(uid_t uid, struct passwd *pwd, char *buf, size_t bufsiz, struct passwd **res) {
#if GETPWUID_R_AIX
                                                                                                                            if (bufsiz == 0)
		return ERANGE;
...
static int
u_getpwnam_r(const char *nam, struct passwd *pwd, char *buf, size_t bufsiz, struct passwd **res) {
#if GETPWNAM_R_AIX
                                                                                                                            if (bufsiz == 0)
		return ERANGE;
...
static u_error_t
u_getsockport(int fd, in_port_t *port, int (*getname)(int, struct sockaddr *, socklen_t *)) {
    union {
...
static ssize_t
u_recvfromto(int fd, void *buf, size_t lim, int flags, struct sockaddr *from, size_t *fromlen,
             struct sockaddr *to, size_t *tolen, u_error_t *error) {
    in_port_t to_port = 0;
    struct iovec iov;
    struct msghdr msg;
    struct cmsghdr *cmsg;
...
static ssize_t
u_sendtofrom(int fd, const void *buf, size_t len, int flags, const struct sockaddr *to,
             size_t tolen, const struct sockaddr *from, size_t fromlen, u_error_t *error) {
    struct iovec iov;
    struct msghdr msg;
    struct cmsghdr *cmsg;
    struct sockaddr_in *in;
 ...
 */
static int unixL_newmetatable(lua_State *L, const char *name, const luaL_Reg *methods,
...
static unixL_State *unixL_loadstate(lua_State *L) {
    ...
    if (LUA_TNIL == lua_rawgetp(L, LUA_REGISTRYINDEX, &cachekey)) {
        ...
        if (lua_type(L, -1) != LUA_TUSERDATA)
            return luaL_error(L, "bad cached unix state context (expected userdata, got %s)",
                              luaL_typename(L, -1)), (void *) NULL;
...
static u_error_t
unixL_getsockname(lua_State *L, int fd, int (*getname)(int, struct sockaddr *, socklen_t *)) {
...
static void unixL_random_buf(lua_State *L, void *buf, size_t bufsiz, const unsigned char *charmap,
                             size_t mapsiz) {
...
static _Bool unixL_integertonumber(unixL_Integer i, lua_Number *p) {
    if (U_ISTFLOAT(lua_Number)) {
        u_static_assert(FLT_RADIX == 2, "FLT_RADIX value unsupported");
        u_static_assert(LUA_NUMBER_MAX_EXP == 0 || UNIXL_INTEGER_PREC < LUA_NUMBER_MAX_EXP,
...
static _Bool unixL_unsignedtonumber(unixL_Unsigned i, lua_Number *p) {
    if (U_ISTFLOAT(lua_Number)) {
        u_static_assert(FLT_RADIX == 2, "FLT_RADIX value unsupported");
        u_static_assert(LUA_NUMBER_MAX_EXP == 0 || UNIXL_UNSIGNED_PREC < LUA_NUMBER_MAX_EXP,
                        "LUA_NUMBER_MAX_EXP too small");
...
static unixL_Integer
(unixL_checkinteger)(lua_State *L, int index, unixL_Integer min, unixL_Integer max) {
...
static unixL_Unsigned
(unixL_checkunsigned)(lua_State *L, int index, unixL_Unsigned min, unixL_Unsigned max) {
    if (lua_isinteger(L, index)) {
        lua_Integer i = lua_tointeger(L, index);
...
static unixL_Integer
unixL_optinteger(lua_State *L, int index, unixL_Integer def, unixL_Integer min, unixL_Integer max) {
...
static u_error_t fd_reopen(int *fd, int ofd, const char *fspath, u_flags_t flags) {
    ...
    if ((error = u_snprintf(path, sizeof path, fspath, (long) getpid(), ofd, (long long) st.st_dev,
                            (long long) st.st_ino, '\0')))
        return error;
...
static u_error_t
fd_mktemp(lua_State *L, int *fd, char *tmpnam, size_t tmpsiz, u_flags_t flags, mode_t mode) {
...
static u_error_t fd_init(lua_State *L, unixL_State *U) {
	 ...
	 */
    if ((error = fd_mktemp(L, &tmpfd, tmpnam, sizeof tmpnam, O_WRONLY | O_APPEND | U_CLOEXEC,
                           0600)))
        goto error;
...
static struct luaL_Stream *unixL_prepfile(lua_State *L) {
    ...
    if (U->lua.jit) {
        ...
        if (U->lua.openf == LUA_NOREF || U->lua.openf == LUA_REFNIL)
            luaL_error(L,
                       "unable to create new file handle: LuaJIT io.open function not available");
...
static void
unixL_checkflags(lua_State *L, int index, const char **mode, u_flags_t *flags, mode_t *perm) {
    index = lua_absindex(L, index);
...
static u_error_t
exec_addtable(lua_State *L, unixL_State *U, size_t *arrp, int index, int anchorindex) {
    size_t i = 0;
    int error = 0;
...
static int unsafe_fcntl(lua_State *L) {
    ...
    return luaL_argerror(L, 3, lua_pushfstring(L, "expected integer or string, got %s",
                                               luaL_typename(L, 3)));
    syerr:
    error = errno;
    error:
...
static int rl_checkrlimit(lua_State *L, int index) {
    ...
    luaL_argcheck(L, i >= 0 && i < (int) countof(what_i), index,
...
static int unix_recvfromto(lua_State *L) {
    ...
    fromlen = sizeof from;
    tolen = sizeof to;
    if (-1 == (n = u_recvfromto(fd, U->buf, size, flags, (struct sockaddr *) &from, &fromlen,
                                (struct sockaddr *) &to, &tolen, &error)))
        return unixL_pusherror(L, error, "recvfromto", "~$#");
...
static int unix_regexec(lua_State *L) {
    ...
    if (top > 2 && lua_istable(L, top)) {
        luaL_argcheck(L, !(re->cflags & REG_NOSUB), top,
    ...
    luaL_argcheck(L, top == 2, top,
                  lua_pushfstring(L, "expected integer or table, got %s", luaL_typename(L, top)));
...
static int unix_sigaction(lua_State *L) {
    ...
    if (lua_toboolean(L, 3)) {
        ...
        *(u_sighandler_t **) lua_newuserdata(L,
                                             sizeof(u_sighandler_t *)) = (u_sighandler_t *) oact.sa_handler;
...
int luaopen_unix(lua_State *L) {
    ...
    for (i = 0; i < countof(unix_const); i++) {
        ...
        for (j = 0; j < size; j++) {
            ...
            if (*table[j].name >= '0' && *table[j].name <= '9')
                return luaL_error(L, "%s: bogus constant identifier string conversion (near %s)",
    ...
    for (i = 0; i < countof(unix_sighandler); i++) {
        ...
        *(u_sighandler_t **) lua_newuserdata(L,
                                             sizeof unix_sighandler[i].func) = unix_sighandler[i].func;
        luaL_setmetatable(L, "sighandler_t*");
