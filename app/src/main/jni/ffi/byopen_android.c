...
by_void_t __system_property_read_callback(
        ...
        by_void_t (*callback)(by_pointer_t cookie, by_char_t const *name, by_char_t const *value,
                              uint32_t serial),
...
static by_void_t
by_rt_prop_read_int(by_pointer_t cookie, by_char_t const *name, by_char_t const *value,
                    uint32_t serial) {
    ...
    if (fp) {
        while (fgets(line, sizeof(line), fp)) {
            if (strstr(line, filename)) {
                ...
                if (3 == sscanf(line, "%"SCNxPTR"-%*"SCNxPTR" %4s %"SCNxPTR" %*x:%*x %*d%n", &start,
