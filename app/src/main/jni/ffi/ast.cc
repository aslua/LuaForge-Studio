...
#define CONVERT_RVAL(lv) { \
    ...
    switch (ret) { \
        ...
        switch (let) {
            case c_expr_type::LDOUBLE: CONVERT_RVAL(ld);
                ret = let;
                return true;
            case c_expr_type::DOUBLE: CONVERT_RVAL(d);
                ret = let;
                return true;
            case c_expr_type::FLOAT: CONVERT_RVAL(f);
                ret = let;
                return true;
            default:
                break;
        ...
        if (lsig == rsig) {
            switch (let) {
                case c_expr_type::ULLONG: CONVERT_RVAL(ull);
                    ret = let;
                    return true;
                case c_expr_type::LLONG: CONVERT_RVAL(ll);
                    ret = let;
                    return true;
                case c_expr_type::ULONG: CONVERT_RVAL(ul);
                    ret = let;
                    return true;
                case c_expr_type::LONG: CONVERT_RVAL(l);
                    ret = let;
                    return true;
                case c_expr_type::UINT: CONVERT_RVAL(u);
                    ret = let;
                    return true;
                case c_expr_type::INT: CONVERT_RVAL(i);
                    ret = let;
                    return true;
                default:
                    break;
