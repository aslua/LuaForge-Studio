    ...
    if (statement_num == 5) {
        ...
        if (lua_isstring(L, 2)) {
            ...
            (*javaEnv)->DeleteLocalRef(javaEnv, text);
                ...
                (javaEnv, (jobject) *(jobject *) lua_touserdata(L, 2), charsequence_class) ==
                   JNI_TRUE) {
...
int drawTextOnPath(lua_State *L) {
    ...
    if (statement_num == 6) {
        ...
        (*javaEnv)->CallVoidMethod(javaEnv, canvas, drawtextonpath2_method,
                                   text2, (jint) index, (jint) count, path, (jfloat) hOffset,
                                   (jfloat) vOffset, paint);
...
int drawVertices(lua_State *L) {
    ...
    if (statement_num == 13) {
        ...
        (*javaEnv)->CallVoidMethod(javaEnv, canvas, drawvertices_method,
                                   mode, (jint) vertexCount, verts, (jint) vertOffset, texs,
                                   (jint) texOffset, colors,
                                   (jint) colorOffset, indices, (jint) indexOffset,
                                   (jint) indexCount, paint);
    } else {
        lua_pushstring(L, "Error. Invalid number of parameters.");
        lua_error(L);
    }
    ...
    if (statement_num == 1) {
        ...
        if (getclipbounds1_method == NULL)
            getclipbounds1_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "getClipBounds",
...
int getMatrix(lua_State *L) {
    ...
    if (statement_num == 1) {
        ...
        if (getmatrix1_method == NULL)
            getmatrix1_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "getMatrix",
    ...
    if (statement_num == 3) {
        ...
        if ((*javaEnv)->IsInstanceOf(javaEnv, who, path_class) == JNI_TRUE) {
            if (quickreject1_method == NULL)
                quickreject1_method =
                        (*javaEnv)->GetMethodID(javaEnv, canvas_class, "quickReject",
...
int saveLayer(lua_State *L) {
    ...
    if (statement_num == 4) {
        ...
        if (savelayer1_method == NULL)
            savelayer1_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "saveLayer",
    ...
    if (statement_num == 4) {
        ...
        if (savelayeralpha1_method == NULL)
            savelayeralpha1_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "saveLayerAlpha",
...
int setBitmap(lua_State *L) {
    ...
    if (statement_num == 2) {
        ...
        if (setbitmap_method == NULL)
            setbitmap_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "setBitmap",
    ...
    if (statement_num == 2) {
        ...
        if (setdrawfilter_method == NULL)
            setdrawfilter_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "setDrawFilter",
...
int setMatrix(lua_State *L) {
    ...
    if (statement_num == 2) {
        ...
        if (setmatrix_method == NULL)
            setmatrix_method =
                    (*javaEnv)->GetMethodID(javaEnv, canvas_class, "setMatrix",
                                            "(Landroid/graphics/Matrix;)V");
