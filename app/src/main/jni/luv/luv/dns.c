...
static int luv_getaddrinfo(lua_State *L) {
    ...
    ret = uv_getaddrinfo(luv_loop(L), req, ref == LUA_NOREF ? NULL : luv_getaddrinfo_cb, node,
                         service, hints);
    if (ret < 0) {
        luv_cleanup_req(L, (luv_req_t *) req->data);
        lua_pop(L, 1);
...
static void
luv_getnameinfo_cb(uv_getnameinfo_t *req, int status, const char *hostname, const char *service) {
    lua_State *L = luv_state(req->loop);
...
static int luv_getnameinfo(lua_State *L) {
    ...
    ret = uv_getnameinfo(luv_loop(L), req, ref == LUA_NOREF ? NULL : luv_getnameinfo_cb,
