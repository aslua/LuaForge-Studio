...
static int lz_assert(lua_State *L, int result, const z_stream *stream, const char *file, int line) {
    ...
    switch (result) {
        ...
        case Z_STREAM_ERROR:
            lua_pushfstring(L,
                            "InternalError: inconsistent internal zlib stream (%s) at %s line %d",
                            stream->msg, file, line);
            break;
        case Z_DATA_ERROR:
            lua_pushfstring(L,
 ...
 */
static int
lz_filter_impl(lua_State *L, int (*filter)(z_streamp, int), int (*end)(z_streamp), char *name) {
    ...
    if (stream == NULL) {
        if (lua_gettop(L) >= 1 && lua_isstring(L, 1)) {
            lua_pushfstring(L,
                            "IllegalState: calling %s function when stream was previously closed",
                            name);
            lua_error(L);
        }
        lua_pushstring(L, "");
...
LUALIB_API int luaopen_zlib(lua_State *const L) {
    ...
    SETLITERAL("_VERSION",
               "lua-zlib $Id: c9f86792ac86380927af9b2393a874e0a2c3f9e5 $  (HEAD, master)");
