...
static void Block(LexState *ls, int *target);
...
static int block_follow(LexState *ls, int withuntil) {
    switch (ls->t.token) {
        ...
        case TK_DEFAULT:
        case TK_CATCH:
        case TK_FINALLY:  /* 添加 try-catch-finally 语法 */
            return 1;
        case TK_UNTIL:
            return withuntil;
        default:
...
static void lambda_body(LexState *ls, expdesc *e, int line) {
    ...
    new_fs.f->linedefined = line;
    open_func(ls, &new_fs, &bl);
    lambda_parlist(ls);                     /* 解析参数列表 */

    /* 根据下一个 token 决定箭头类型 */
    if (testnext(ls, TK_DARROW)) {          /* '=>' 形式 */
        if (testnext(ls, TK_DO)) {          /* 块体：=> do ... end */
            block(ls);                      /* 解析块内语句 */
            check_match(ls, TK_END, TK_DO, ls->linenumber); /* 消耗 end */
        } else {                             /* 表达式体：=> expr */
            expdesc ret_exp;
            expr(ls, &ret_exp);
            luaK_exp2nextreg(ls->fs, &ret_exp);
            luaK_ret(ls->fs, ret_exp.u.info, 1);
        }
    } else if (testnext(ls, TK_MEAN)) {     /* '->' 形式 */
        if (testnext(ls, TK_DO)) {          /* 块体：-> do ... end */
            block(ls);
            check_match(ls, TK_END, TK_DO, ls->linenumber);
        } else {                             /* 表达式体：-> expr */
            expdesc ret_exp;
            expr(ls, &ret_exp);
            luaK_exp2nextreg(ls->fs, &ret_exp);
        ...
        luaX_syntaxerror(ls, "expected '->' or '=>' after lambda parameters");
    }
...
static void suffixedexp(LexState *ls, expdesc *v) {
    ...
    for (;;) {
        switch (ls->t.token) {
            ...
            case '[': {  /* '[' exp ']' */
                expdesc key;
                luaK_exp2anyregup(fs, v);  /* 确保表格在寄存器中 */
                yindex(ls, &key);  /* 读取索引表达式 */
                luaK_indexed(fs, v, &key);  /* 生成索引代码 */
                break;
            }
            case ':': {  /* ':' NAME funcargs */
                /* 如果处于三元运算符上下文中（需要匹配 ':'），不要处理方法调用 */
                if (ls->ternary_nest > 0) {
                    return;  /* 返回，让上层三元运算符处理这个 ':' */
                }
            ...
            case TK_OPTDOT: {  /* ?. */
                ...
                if (ls->t.token == '[') {                /* 可选索引 ?.[ */
                    ...
                    luaX_next(ls);  /* skip '[' */
                    expr(ls, &key);
                    check_match(ls, ']', '[', line);     /* 消耗 ']' */
                    luaK_optindex(ls->fs, v, &key, line);
                } else if (ls->t.token == '(') {         /* 可选调用 ?.( */
                    /* 可选调用支持可在此扩展 */
                    luaX_syntaxerror(ls, "optional call not implemented yet");
                } else {                                  /* 可选字段 ?.name */
                    expdesc key;
                    codename(ls, &key);
                    luaK_optfield(ls->fs, v, &key, line);
                }
                break;
            }
            default:
...
static BinOpr subexpr(LexState *ls, expdesc *v, int limit) {
    ...
    if (ls->t.token == TK_QUESTION && limit < 1) {
        FuncState *fs = ls->fs;
        int line = ls->linenumber;
        ...
        expdesc then_exp;
        subexpr(ls, &then_exp, 0);
        luaK_exp2nextreg(fs, &then_exp);
        int reg = then_exp.u.info;   // 获取 then 结果的寄存器号

        int exit_jump = luaK_jump(fs);
        ...
        expdesc else_exp;
        subexpr(ls, &else_exp, 0);
        luaK_exp2nextreg(fs, &else_exp);
        int else_reg = else_exp.u.info;   // 获取 else 结果的寄存器号

        if (else_reg != reg) {
            luaK_codeABC(fs, OP_MOVE, reg, else_reg, 0);
        }

        luaK_patchtohere(fs, exit_jump);
...
/*
** Convert compound assignment token to binary operator
*/
static BinOpr compoundassign_to_binopr(int token) {
    switch (token) {
        case TK_ADD_ASSIGN:
            return OPR_ADD;
        case TK_SUB_ASSIGN:
...
/*
** Process compound assignment: var op= exp
*/
static void compoundassign(LexState *ls, struct LHS_assign *lh, int token) {
    FuncState *fs = ls->fs;
    BinOpr opr = compoundassign_to_binopr(token);
    int line = ls->linenumber;

    if (opr == OPR_NOBINOPR) {
        luaX_syntaxerror(ls, "invalid compound assignment operator");
    }
    if (lh->prev != NULL) {
        luaX_syntaxerror(ls, "compound assignment can only be used with single variable");
    }
    ...
    if (vkisindexed(kind)) {
        int t_reg = var->u.ind.t;
        int idx_val = var->u.ind.idx;
        int is_upvalue = (kind == VINDEXUP);
        ...
        int new_t_reg, new_idx_val;
        if (is_upvalue) {
            /* upvalue 不需要复制，直接使用原始值 */
            new_t_reg = t_reg;
            new_idx_val = idx_val;
        } else {
            /* 复制表到临时寄存器 */
            new_t_reg = base_reg;
            luaK_codeABC(fs, OP_MOVE, new_t_reg, t_reg, 0);
            if (kind == VINDEXED) {
                /* 索引也是寄存器，需要复制 */
                new_idx_val = base_reg + 1;
                luaK_codeABC(fs, OP_MOVE, new_idx_val, idx_val, 0);
        ...
        luaK_reserveregs(fs, 1);
        if (get_k >= 0) {
            luaK_codeABCk(fs, get_op, val_reg, new_t_reg, new_idx_val, get_k);
        } else {
            luaK_codeABC(fs, get_op, val_reg, new_t_reg, new_idx_val);
        }
        luaK_fixline(fs, line);

        /* 创建临时表达式表示字段当前值 */
        expdesc temp;
        init_exp(&temp, VNONRELOC, val_reg);

        /* 执行复合运算 */
        luaK_infix(fs, opr, &temp);
        luaK_posfix(fs, opr, &temp, &right, line);

        /* 确保结果在寄存器中 */
        luaK_exp2anyreg(fs, &temp);

        /* 更新 var 的索引字段，使其指向我们实际使用的表/索引位置 */
        if (!is_upvalue) {
            var->u.ind.t = new_t_reg;
            if (kind == VINDEXED) {
                var->u.ind.idx = new_idx_val;
            }
            /* 对于 VINDEXSTR 和 VINDEXI，idx 是常量，不需要更新 */
        }

        /* 将结果存储回原变量 */
        luaK_storevar(fs, var, &temp);
    ...
    /* 处理普通变量（局部、upvalue、全局） */
    expdesc left, right;
    left = *var;
    luaK_dischargevars(fs, &left);
    if (left.k != VNONRELOC) {
        luaK_exp2anyreg(fs, &left);
    }
    expr(ls, &right);
    luaK_infix(fs, opr, &left);
    luaK_posfix(fs, opr, &left, &right, line);
    if (left.k != VNONRELOC && left.k != VVOID) {
        luaK_exp2anyreg(fs, &left);
    }
    luaK_storevar(fs, var, &left);
}
...
static void restassign(LexState *ls, struct LHS_assign *lh, int nvars) {
    ...
    if (ls->t.token == TK_ADD_ASSIGN || ls->t.token == TK_SUB_ASSIGN ||
        ls->t.token == TK_MUL_ASSIGN || ls->t.token == TK_DIV_ASSIGN ||
        ls->t.token == TK_MOD_ASSIGN || ls->t.token == TK_POW_ASSIGN ||
        ls->t.token == TK_CONCAT_ASSIGN || ls->t.token == TK_BAND_ASSIGN ||
        ls->t.token == TK_BOR_ASSIGN ||
        ls->t.token == TK_SHL_ASSIGN || ls->t.token == TK_SHR_ASSIGN ||
        ls->t.token == TK_IDIV_ASSIGN) {
...
static void gotostat(LexState *ls) {
    ...
    } else if (ls->t.token >= FIRST_RESERVED && ls->t.token < TK_EOS) {
        /* 从 token 数组获取保留字的字符串 */
        const char *tokenstr = luaX_tokens[ls->t.token - FIRST_RESERVED];
        name = luaX_newstring(ls, tokenstr, strlen(tokenstr));
        luaX_next(ls);  /* 消耗这个 token */
    } else {
    ...
    if (lb == NULL)  /* 没有找到标签？ */
        ...
        /* 后向跳转；立即解析 */
        int lblevel = reglevel(fs, lb->nactvar);  /* 标签所在的层级 */
        if (luaY_nvarstack(fs) > lblevel)  /* 离开变量作用域？ */
...
static void whenstat(LexState *ls, int line) {
    ...
    FuncState *fs = ls->fs;
    int escapelist = NO_JUMP;  /* exit list for finished parts */
    single_test_then_block(ls, &escapelist);  /* first WHEN branch */
    while (ls->t.token == TK_CASE)
        single_test_then_block(ls, &escapelist);  /* subsequent CASE branches */
    if (testnext(ls, TK_ELSE))
        single_block(ls);  /* ELSE part */
    check_match(ls, TK_END, TK_WHEN, line);  /* <--
检查并消耗 END */
    luaK_patchtohere(fs, escapelist);  /* patch all escapes to here */
}
...
static void exprstat(LexState *ls) {
    ...
    /* 允许复合赋值运算符进入赋值分支 */
    if (ls->t.token == '=' || ls->t.token == ',' ||
        ls->t.token == TK_ADD_ASSIGN || ls->t.token == TK_SUB_ASSIGN ||
        ls->t.token == TK_MUL_ASSIGN || ls->t.token == TK_DIV_ASSIGN ||
        ls->t.token == TK_MOD_ASSIGN || ls->t.token == TK_POW_ASSIGN ||
        ls->t.token == TK_CONCAT_ASSIGN || ls->t.token == TK_BAND_ASSIGN ||
        ls->t.token == TK_BOR_ASSIGN ||
        ls->t.token == TK_SHL_ASSIGN || ls->t.token == TK_SHR_ASSIGN ||
        ls->t.token == TK_IDIV_ASSIGN) {
        /* stat -> assignment or compound assignment */
        v.prev = NULL;
        restassign(ls, &v, 1);
    } else {  /* stat -> func */
        Instruction *inst;
...
static void trystat(LexState *ls, int line) {
    ...
    if (ls->t.token == TK_CATCH) {
        ...
        /* 必须紧跟 '(' */
        if (ls->t.token != '(')
            luaX_syntaxerror(ls, "'(' expected after 'catch'");
        luaX_next(ls);  /* skip '(' */

        /* 括号内必须有一个标识符作为错误变量 */
        if (ls->t.token != TK_NAME)
            error_expected(ls, TK_NAME);
        TString *err_name = str_checkname(ls);  /* 消耗标识符 */
...
*/
static void deferstat(LexState *ls, int line) {
    /* defer stat -> DEFER block END */
    luaX_next(ls);  /* skip DEFER */

    /* 创建新函数原型以包含延迟块 */
    FuncState *parent_fs = ls->fs;
    FuncState new_fs;
    BlockCnt bl;
    expdesc closure;
    ...
    /* 解析块（block），块以 END 结束 */
    block(ls);
    check_match(ls, TK_END, TK_DEFER, line);

    new_fs.f->lastlinedefined = ls->linenumber;
    codeclosure(ls, &closure);  /* 生成闭包指令，结果在 closure 中 */
    close_func(ls);  /* 关闭新函数，恢复 ls->fs 为 parent_fs */

    /* 回到父函数，生成调用 defer(closure) 的代码 */
    FuncState *fs = parent_fs;

    /* 获取全局函数 defer */
    expdesc defer_func;
    singlevaraux(fs, luaS_newliteral(ls->L, "defer"), &defer_func, 1);
    if (defer_func.k == VVOID) {
        /* 如果未找到，从环境获取 */
        expdesc key;
        singlevaraux(fs, ls->envn, &defer_func, 1);
        codestring(&key, luaS_newliteral(ls->L, "defer"));
        luaK_indexed(fs, &defer_func, &key);
    }
    luaK_exp2nextreg(fs, &defer_func);
    int base = defer_func.u.info;

    /* 将闭包放入下一个寄存器 */
    luaK_exp2nextreg(fs, &closure);

    /* 调用 defer(closure) */
    luaK_codeABC(fs, OP_CALL, base, 2, 1);  /* 1个参数，0个结果（C=1） */
    luaK_fixline(fs, line);
...
static void statement(LexState *ls) {
    ...
    switch (ls->t.token) {
        ...
        case TK_DBCOLON: {  /* stat -> label */
            ...
            checknext(ls, TK_DBCOLON);  /* 跳过第二个 '::' */
            while (ls->t.token == ';' || ls->t.token == TK_DBCOLON)
                statement(ls);  /* 跳过其他空语句 */
            checkrepeated(ls, name);  /* 检查重复标签 */
            createlabel(ls, name, line, block_follow(ls, 0));
            break;
        }
        ...
        case TK_TRY: {  /* 添加 try-catch-finally 语句 */
            trystat(ls, line);
            break;
        }
        case TK_DEFER: {
