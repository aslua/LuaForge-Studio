...
static int str_split(lua_State *L) {
    ...
    lua_Integer limit = luaL_optinteger(L, 3, 0);
    int match_limited = (limit > 0);
    ...
    if (sep_len == 0) {
        ...
        for (size_t i = 0; i < src_len; i++) {
            if (match_limited && result_index >= limit - 1) {
                // 达到限制，添加剩余部分
                lua_pushlstring(L, src + i, src_len - i);
        ...
        if (!match_limited || result_index < limit) {
            // 添加最后一个部分
            lua_pushlstring(L, src, src_len);
            lua_rawseti(L, -2, ++result_index);
        }
    } else {
        ...
        while (start < end) {
            ...
            if (sep_len == 1) {
                // 单个字符分隔符，使用 memchr
                match = (const char *) memchr(start, sep[0], end - start);
            } else {
                // 多字符分隔符，手动查找
                for (const char *p = start; p <= end - sep_len; p++) {
            ...
            if (match) {
                ...
                if (match > start || match == start) {
                    lua_pushlstring(L, start, match - start);
                    lua_rawseti(L, -2, ++result_index);
                }
                start = match + sep_len;
            } else {
    ...
    // 移除末尾的空字符串（如果limit为0）
    if (limit == 0 && result_index > 0) {
        lua_rawgeti(L, -1, result_index);
        size_t len;
        lua_tolstring(L, -1, &len);
        lua_pop(L, 1);  // 弹出检查的字符串

        while (result_index > 0 && len == 0) {
            lua_pushnil(L);
            lua_rawseti(L, -2, result_index);
            result_index--;
            if (result_index > 0) {
