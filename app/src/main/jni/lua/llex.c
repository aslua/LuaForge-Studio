...
const char *const luaX_tokens[] = {
        ...
        "global", "goto", "if", "in", "lambda", "local", "nil",
        "not", "or", "repeat",
        "return", "switch", "then", "true", "until", "when", "while",

        /* try-catch-finally */
        "try", "catch", "finally", "defer",      // <-- 新增 "defer"
        ...
        "let", "->", "=>",            /* TK_LET, TK_MEAN, TK_DARROW 按此顺序 */
...
const char *luaX_token2str(LexState *ls, int token) {
    if (token == TK_LAMBDA) return "\\";
    if (token == TK_MEAN) return "->";
    if (token == TK_DARROW) return "=>";
...
static int read_numeral(LexState *ls, SemInfo *seminfo) {
    ...
    if (first == '0' && (ls->current == 'b' || ls->current == 'B')) {
        save_and_next(ls);  /* 保存 'b' 或 'B' */

        /* 解析二进制数字 */
        lua_Integer num = 0;
        int has_digit = 0;

        while (ls->current == '0' || ls->current == '1' || ls->current == '_') {
            if (ls->current != '_') {
                num = num * 2 + (ls->current - '0');
                has_digit = 1;
            }
            save_and_next(ls);
        }
        ...
        switch (ls->current) {
            ...
            case '/': {
                ...
                if (ls->current == '=') {  // '/='
                    ...
                    if (ls->current == '=') {  // '//='
                        next(ls);
                        return TK_IDIV_ASSIGN;   // 返回整数除法赋值
                    }
                    return TK_IDIV;   // 普通的整数除法运算符
                } else return '/';
            }
            case '~': {
                next(ls);
            ...
            case '&': {
                ...
                } else if (check_next1(ls, '&')) return TK_AND;
                else return '&';
            }
            case '?': {
                next(ls);
            ...
            case '\\': { /* 识别 \ 作为 lambda 起始符 */
                next(ls);
                return TK_LAMBDA;
            }
            default: {
                if (lislalpha(ls->current) ||
                    ls->current >= 0x80) {  /* identifier or reserved word? */
                    TString *ts;
                    do {
                        save_and_next(ls);
